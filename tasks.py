...
def coverage(c):
    ...
    """
    c.run("coverage run --source dj-beneficiary runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/dj-beneficiary.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ dj_beneficiary")
...
def lint(c):
    ...
    """
    c.run("flake8 dj-beneficiary tests")
...
def release(c, bumpsize=''):
    ...
    import dj_beneficiary
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=dj_beneficiary.__version__))
    c.run("git push --tags")
